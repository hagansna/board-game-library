...
export const load: PageServerLoad = async ({ parent, params, locals }) => {
	...
	// Fetch the game from the catalog
	const game = await getGameById(locals.supabase, gameId);
...
export const actions: Actions = {
	default: async ({ request, params, locals }) => {
		...
		// Update the game in the shared catalog
		...
		const game = await updateSharedGame(locals.supabase, gameId, gameData);
		...
		if (!game) {
			return fail(500, {
				...
				bggRank: bggRankStr,
				suggestedAge: suggestedAgeStr,
				errors: { general: 'Failed to update game. Please try again.' }
			});
		}
